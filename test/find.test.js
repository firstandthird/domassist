...
test("find - string selector", (assert) => {
  ...
  assert.equal(
    els.length,
    total,
    `Number of returned items should be ${total}`
  );
  teardown(el);
  assert.end();
});
...
test("find - from document", (assert) => {
  ...
  assert.equal(
    el,
    document.body,
    "Should have a context of document if none provided."
  );
  assert.end();
});
...
test("find - DOM Node", (assert) => {
  const el = document.getElementById("domassist");
  assert.ok(
    Array.isArray(domassist.find(el)),
    "Passed DOM node is returned as an array"
  );
  assert.equal(
    domassist.find(el).length,
    1,
    "Number of returned items should be 1"
  );
  assert.equal(
    domassist.find(window)[0],
    window,
    "It works with the window object"
  );
...
test("find - invalid selector", (assert) => {
  assert.equal(
    domassist.find("hello").length,
    0,
    "Pass non-existent selector returns empty array"
  );
  assert.end();
});
...
test("find - element with context", (assert) => {
  ...
  assert.ok(
    Array.isArray(para),
    "Pass selector with context should return an array"
  );
  assert.equal(para.length, 1, "Number of return items should be 1");
  assert.equal(
    para[0].innerHTML,
    "paragraph-1",
    'Element\'s copy should be "paragraph-1"'
  );
  teardown(el);
  assert.end();
});
