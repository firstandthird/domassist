...
test("addClass, hasClass, removeClass, toggleClass", (assert) => {
  ...
  assert.ok(
    domassist.hasClass(el.firstChild, "new-class"),
    'toggleClass - class list should contain "new-class"'
  );
  domassist.toggleClass(el.firstChild, "new-class");
  assert.notOk(
    domassist.hasClass(el.firstChild, "new-class"),
    'toggleClass - class list should not contain "new-class"'
  );
...
test("removeClass - single element / multiple classes", (assert) => {
  ...
  assert.equal(
    el.classList.length,
    0,
    "Three classes removed from single element"
  );
  assert.end();
});
...
test("hasClass - selector", (assert) => {
  ...
  setup(el);
  assert.equal(
    domassist.hasClass(".test-divs", "test-divs"),
    true,
    "selector with existing element"
  );
  assert.equal(
    domassist.hasClass(".nope", "test-divs"),
    false,
    "selector with non-existing element"
  );
  ...
  assert.equal(
    domassist.hasClass(nonEl, "test-divs"),
    false,
    "false if element doesnt exist"
  );
  teardown(el);
  assert.end();
});
