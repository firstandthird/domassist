...
test("remove, selector in context", (assert) => {
  ...
  assert.equal(
    domassist.find(".content").length,
    2,
    "elements exist before removing"
  );
  domassist.remove(".content", container);
  assert.equal(domassist.find(".content").length, 1, "element was removed");
  assert.equal(
    domassist.findOne(".content").parentNode.id,
    "domassist",
    "element was removed from its context"
  );
...
test("remove, not matched selector", (assert) => {
  ...
  assert.doesNotThrow(
    () => domassist.remove(".new"),
    "no error thrown on non-matched selector"
  );
  teardown(el);
  assert.end();
});
